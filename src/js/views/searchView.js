...
const renderButtons = (page, numOfResults, resPerPage) => {
  ...
  if (page === 1 && pages > 1) {
    ...
    // 1. button to go to previous page only. bcz its for only last page
    button = createButtons(page, "prev");
  }

  //adding buttons to parent div
  elements.searchResPages.insertAdjacentHTML("afterbegin", button);
};

export const renderResults = (recipes, page = 1, resPerPage = 10) => {
  //render results of current page
  const start = (page - 1) * resPerPage; //page3 = start 20 => (3-1)*10
